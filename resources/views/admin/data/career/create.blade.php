...
<div class="content ml-12 transform ease-in-out duration-500 pt-20 px-2 md:px-5 pb-4">
    ...
    <div class="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg my-10">
        <h2 class="text-2xl font-bold mb-6">Tambah Karir Baru</h2>
        <form action="{{ route('admin.career.store') }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="kode_karir" id="kode_karir" required
                    ...
                    placeholder="Masukkan kode karir" value="{{ old('kode_karir', $nextKode ?? '') }}">
                @error('kode_karir')
            ...
            <div class="mb-5">
                ...
                <input type="text" name="nama_karir" id="nama_karir" required
                    class="bg-gray-50 focus:outline-none focus:ring-1 border border-indigo-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    placeholder="Masukkan nama karir" value="{{ old('nama_karir') }}">
                @error('nama_karir')
                <div class="text-red-600">
                    {{ $message }}
                </div>
            ...
            <div class="mb-5">
                ...
                <select name="tipe_kepribadian" id="tipe_kepribadian" required
                    ...
                    <option value="Realistic" {{ old('tipe_kepribadian')=='Realistic' ? 'selected' : '' }}>Realistic
                    </option>
                    <option value="Investigative" {{ old('tipe_kepribadian')=='Investigative' ? 'selected' : '' }}>
                        Investigative</option>
                    <option value="Artistic" {{ old('tipe_kepribadian')=='Artistic' ? 'selected' : '' }}>Artistic
                    </option>
                    <option value="Social" {{ old('tipe_kepribadian')=='Social' ? 'selected' : '' }}>Social</option>
                    <option value="Enterprising" {{ old('tipe_kepribadian')=='Enterprising' ? 'selected' : '' }}>
                        Enterprising</option>
                    <option value="Conventional" {{ old('tipe_kepribadian')=='Conventional' ? 'selected' : '' }}>
                        Conventional</option>
                </select>
            ...
            <div class="mb-5">
                <label for="solusi" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Solusi
                    Karir</label>
                <textarea name="solusi" id="solusi" required rows="5"
                    class="bg-gray-50 focus:outline-none focus:ring-1 border border-indigo-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
